...
const Signup = () => {
  ...
  const validate = () => {
    ...
    if (form.password.length < 5) errs.password = 'Minimum 5 characters';
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (err.response) {
        setErrors({ submit: err.response.data?.message || `Error: ${err.response.status}` });
      } else if (err.request) {
        setErrors({ submit: 'No response from server. Try again later.' });
      } else {
